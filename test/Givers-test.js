...
describe("Givers Chain", function () {
  beforeEach(async function () {
    ...
    hardhatToken = await GiversToken.deploy(
      addr1.address,
      addr2.address,
  ...
  describe("Deployment", function () {
    ...
    it("Should assign the total supply of tokens to the owner", async function () {
      const ownerBalance =
        ((await hardhatToken.balanceOf(owner.address)) / 10 ** 18) * 10 ** 9;
      expect((await hardhatToken.totalSupply()) / 10 ** 18).to.equal(10 ** 9);
    });
  });
  ...
  describe("Transfers:Before swapAndLiquify is enabled", function () {
    ...
    it("Should transfer with fee for included accounts ", async function () {
      ...
      await hardhatToken
        .connect(addr3)
        .transfer(addr4.address, ethers.utils.parseEther("50"));
      ...
      const reflect =
        (addr4Balance - ethers.utils.parseEther("45")) /
      ...
      await hardhatToken
        .connect(addr4)
        .transfer(addr5.address, ethers.utils.parseEther("30"));
      ...
      const reflection2 =
        (addr5Balance - ethers.utils.parseEther("27")) /
        ethers.utils.parseEther("27");

      expect(true).to.be.equal(
        ethers.utils.parseEther("15") < addr4Balance2 &&
          addr4Balance2 < ethers.utils.parseEther("16")
      );
      ...
      await hardhatToken
        .connect(addr5)
        .transfer(addr6.address, ethers.utils.parseEther("20"));
      ...
      const reflection3 =
        (addr6Balance - ethers.utils.parseEther("27")) /
        ethers.utils.parseEther("27");

      expect(true).to.equal(
        ethers.utils.parseEther("7") < addr5Balance2 &&
          addr5Balance2 < ethers.utils.parseEther("8")
      );
    ...
    it("should send fee to burn wallent", async function () {
      const burnBalanceBefore = await hardhatToken.balanceOf(
      ...
      await hardhatToken
        .connect(addr3)
        .transfer(addr4.address, ethers.utils.parseEther("200"));
      const burnBalanceAfter = await hardhatToken.balanceOf(
        "0x000000000000000000000000000000000000dEaD"
      );

      expect(true).to.be.equal(
        burnBalanceAfter > ethers.utils.parseEther("2") &&
          burnBalanceAfter < ethers.utils.parseEther("3")
      );
    });
  });
  ...
  describe("Transfers:After swapAndLiquify is enabled", function () {
    ...
    it("Should transfer with fee for included accounts ", async function () {
      ...
      await hardhatToken
        .connect(addr3)
        .transfer(addr4.address, ethers.utils.parseEther("50000000"));
      const addr3Balance2 = await hardhatToken.balanceOf(addr3.address);
      const addr4Balance = await hardhatToken.balanceOf(addr4.address);
      const reflect =
        (addr4Balance - ethers.utils.parseEther("45000000")) /
      ...
      await hardhatToken
        .connect(addr4)
        .transfer(addr5.address, ethers.utils.parseEther("30000000"));
      const addr4Balance2 = await hardhatToken.balanceOf(addr4.address);
      const addr5Balance = await hardhatToken.balanceOf(addr5.address);
      const reflection2 =
        (addr5Balance - ethers.utils.parseEther("27000000")) /
        ethers.utils.parseEther("30000000");

      expect(true).to.be.equal(
        ethers.utils.parseEther("15000000") < addr4Balance2 &&
          addr4Balance2 < ethers.utils.parseEther("16000000")
      );
      ...
      await hardhatToken
        .connect(addr5)
        .transfer(addr6.address, ethers.utils.parseEther("20000000"));
      const addr5Balance2 = await hardhatToken.balanceOf(addr5.address);
      const addr6Balance = await hardhatToken.balanceOf(addr6.address);
      const reflection3 =
        (addr6Balance - ethers.utils.parseEther("18000000")) /
        ethers.utils.parseEther("20000000");

      expect(true).to.equal(
        ethers.utils.parseEther("7000000") < addr5Balance2 &&
          addr5Balance2 < ethers.utils.parseEther("8000000")
      );
      expect(true).to.equal(0 < reflection3 && reflection3 < 3);
    ...
    it("Should send fee to Charity and marketing wallet", async function () {
      ...
      const charityBalanceBefore = await this.provider.getBalance(
        addr1.address
      );
      const marketingBalanceBefore = await this.provider.getBalance(
        addr2.address
      );
      const tokenContractBalanceBefore = await this.provider.getBalance(
        hardhatToken.address
      );
      ...
      const marketingBalanceAfter = await this.provider.getBalance(
        addr2.address
      );
      const tokenContractBalanceAfter = await this.provider.getBalance(
        hardhatToken.address
      );

      const marketingDiff =
        (marketingBalanceAfter - marketingBalanceBefore) / 10 ** 18;
      const charityDiff =
        (charityBalanceAfter - charityBalanceBefore) / 10 ** 18;
      const tokenDiff =
        (tokenContractBalanceAfter - tokenContractBalanceBefore) / 10 ** 18;

      expect(0).to.be.equal(Math.round(marketingDiff));
      expect(7).to.be.equal(Math.round(charityDiff));
      expect(3.5).to.be.equal(Math.round(tokenDiff * 10) / 10);
    });
  });
  ...
  describe("Liquidity", function () {
    it("Should add right amount of liquidty", async function () {
      ...
      expect(totalLiquidity / 10 ** decimals).to.be.equal(
        Ownerliquidity / 10 ** decimals
      );
